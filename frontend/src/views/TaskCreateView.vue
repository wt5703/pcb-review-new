...
import type { TaskFileReference } from '@/api/types'
...
function selectFiles(event: Event): void { sourceFiles.value = Array.from((event.target as HTMLInputElement).files ?? []); uploadedFiles.value = [] }
async function save(submitNow: boolean): Promise<void> {
  error.value = ''; saving.value = true
  try {
    if (sourceFiles.value.length && !uploadedFiles.value.length) uploadedFiles.value = await reviewApi.uploadInitialFilesToCompany(sourceFiles.value)
