...
<template>
  ...
  <section class="card filters task-filters"><div class="keyword-pair"><input v-model="form.taskName" placeholder="任务名称" @keyup.enter="load()" /><input v-model="form.projectName" placeholder="项目名称" @keyup.enter="load()" /></div>
    ...
    <select v-model="form.status"><option value="">当前状态</option><option value="DRAFT">草稿</option><option value="PCB_EXPERT_REVIEWING">专家评审中</option><option value="MUTUAL_REVIEWING">互检中</option><option value="PENDING_FINISH_CONFIRMATION">待结束确认</option><option value="FINISHED">已结束</option></select><input v-model="form.designerName" placeholder="设计者（模糊查询）" @keyup.enter="load()" /><button class="btn primary" @click="load()">查询</button></section>
