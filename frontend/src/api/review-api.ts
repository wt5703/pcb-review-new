...
import type { Archive, AssignableReviewerRole, CheckItem, CheckItemCategory, FileReference, TaskFileReference, MyTask, Opinion, OpinionPage, OpinionSummary, ResourceUploadResult, Reviewer, Task, TaskFile, TaskPage, TemplateCategory, TemplateListCategory } from './types'
...
export const reviewApi = {
  ...
  transition: (taskId: number, body: { actions: string[]; comment?: string; assignedRole?: string; reviewerIds?: number[]; stageFiles?: Array<FileReference & { scene: 'PROCESS_REVIEW' | 'PCB_REVIEW' | 'SCHEMATIC_REVIEW'; fileKind?: 'PROCESS' | 'STRUCTURE' }> }) =>
  ...
  listReviewerWhitelists: () => request<Array<{ id: number; reviewRole: 'HARDWARE_EXPERT' | 'EMC_EXPERT' | 'STRUCTURE_EXPERT' | 'PROCESS_EXPERT' | 'PCB_EXPERT'; employeeNo: string; displayName?: string; email?: string; mobile?: string; createdAt?: string }>>('/reviewer-whitelists'),
