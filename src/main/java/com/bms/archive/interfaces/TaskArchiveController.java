...
@Tag(name = "任务归档", description = "读取已结束评审任务的不可变归档快照，返回流程节点、阶段文件和邮件记录。")
public class TaskArchiveController {
    ...
    @GetMapping
    @Operation(summary = "查询任务归档记录", description = "仅可读取已结束任务的归档快照；返回流程节点时间/节点名称/操作人姓名/流转意见，以及阶段文件和邮件记录。不返回任务快照、评审人员或评审意见。")
