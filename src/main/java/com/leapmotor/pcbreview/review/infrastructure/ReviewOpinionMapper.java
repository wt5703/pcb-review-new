...
public interface ReviewOpinionMapper {
    ...
    @Select("SELECT id, task_id AS taskId, source_type AS sourceType, source_item_id AS sourceItemId, severity, content, raised_by AS raisedBy, "
            + "file_version_id AS fileVersionId, status, version FROM review_opinion WHERE id=#{id}")
    ReviewOpinionRecord findById(long id);

    @Select("SELECT id, task_id AS taskId, source_type AS sourceType, source_item_id AS sourceItemId, severity, content, raised_by AS raisedBy, "
            + "file_version_id AS fileVersionId, status, version FROM review_opinion WHERE task_id=#{taskId} ORDER BY id")
    ...
    @Select("SELECT id, opinion_id AS opinionId, reply_type AS replyType, reason, file_version_id AS fileVersionId, replied_by AS repliedBy, reply_no AS replyNo "
